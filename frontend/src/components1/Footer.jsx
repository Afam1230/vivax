
import { Box, Container, Text, Flex, VStack, HStack, Link, SimpleGrid, Divider, Icon, Input, Button, useToast } from "@chakra-ui/react";
import { EmailIcon } from "@chakra-ui/icons";

const Footer = () => {
    const toast = useToast();
  
    const handleSubmit = (e) => {
      e.preventDefault();
    
      setTimeout(() => {
        toast({
          title: "Thanks for subscribing!",
          description: "We would be sure to keep you updated with our crypto insights.",
          status: "success",
          duration: 4000,
          isClosable: true,
          position: "top",
        });
      }, 1500);
    };
  return (
    <Box bg="gray.900" color="white" borderTop="1px solid" borderColor="gray.800">
      <Container maxW="container.xl" py={10}>
        <SimpleGrid columns={{ base: 1, md: 2, lg: 4 }} spacing={8} mb={10}>
          <VStack align="flex-start" spacing={4}>
            <Flex align="center">
              <Box boxSize={8} bg="purple.500" borderRadius="full" mr={2}></Box>
              <Box fontWeight="bold" fontSize="xl">CryptoLab</Box>
            </Flex>
            <Text color="gray.400">
              The most advanced cryptocurrency mining platform designed for maximum returns and seamless user experience.
            </Text>
            <HStack spacing={4}>
              <Box as="a" href="#" aria-label="Twitter" rounded="full" w={8} h={8} bg="gray.800" display="flex" alignItems="center" justifyContent="center">
                <Icon boxSize={4} viewBox="0 0 24 24">
                  <path fill="currentColor" d="M22.46,6C21.69,6.35 20.86,6.58 20,6.69C20.88,6.16 21.56,5.32 21.88,4.31C21.05,4.81 20.13,5.16 19.16,5.36C18.37,4.5 17.26,4 16,4C13.65,4 11.73,5.92 11.73,8.29C11.73,8.63 11.77,8.96 11.84,9.27C8.28,9.09 5.11,7.38 3,4.79C2.63,5.42 2.42,6.16 2.42,6.94C2.42,8.43 3.17,9.75 4.33,10.5C3.62,10.5 2.96,10.3 2.38,10C2.38,10 2.38,10 2.38,10.03C2.38,12.11 3.86,13.85 5.82,14.24C5.46,14.34 5.08,14.39 4.69,14.39C4.42,14.39 4.15,14.36 3.89,14.31C4.43,16 6,17.26 7.89,17.29C6.43,18.45 4.58,19.13 2.56,19.13C2.22,19.13 1.88,19.11 1.54,19.07C3.44,20.29 5.7,21 8.12,21C16,21 20.33,14.46 20.33,8.79C20.33,8.6 20.33,8.42 20.32,8.23C21.16,7.63 21.88,6.87 22.46,6Z" />
                </Icon>
              </Box>
              <Box as="a" href="#" aria-label="Discord" rounded="full" w={8} h={8} bg="gray.800" display="flex" alignItems="center" justifyContent="center">
                <Icon boxSize={4} viewBox="0 0 24 24">
                  <path fill="currentColor" d="M22,24L16.75,19L17.38,21H4.5A2.5,2.5 0 0,1 2,18.5V3.5A2.5,2.5 0 0,1 4.5,1H19.5A2.5,2.5 0 0,1 22,3.5V24M12,6.8C9.32,6.8 7.44,7.95 7.44,7.95C8.47,7.03 10.27,6.5 10.27,6.5L10.1,6.33C8.41,6.36 6.88,7.53 6.88,7.53C5.16,11.12 5.27,14.22 5.27,14.22C6.67,16.03 8.75,15.9 8.75,15.9L9.46,15C8.21,14.73 7.42,13.62 7.42,13.62C7.42,13.62 9.3,14.9 12,14.9C14.7,14.9 16.58,13.62 16.58,13.62C16.58,13.62 15.79,14.73 14.54,15L15.25,15.9C15.25,15.9 17.33,16.03 18.73,14.22C18.73,14.22 18.84,11.12 17.12,7.53C17.12,7.53 15.59,6.36 13.9,6.33L13.73,6.5C13.73,6.5 15.53,7.03 16.56,7.95C16.56,7.95 14.68,6.8 12,6.8M9.93,10.59C10.58,10.59 11.11,11.16 11.1,11.86C11.1,12.55 10.58,13.13 9.93,13.13C9.29,13.13 8.77,12.55 8.77,11.86C8.77,11.16 9.28,10.59 9.93,10.59M14.1,10.59C14.75,10.59 15.27,11.16 15.27,11.86C15.27,12.55 14.75,13.13 14.1,13.13C13.46,13.13 12.94,12.55 12.94,11.86C12.94,11.16 13.45,10.59 14.1,10.59Z" />
                </Icon>
              </Box>
              <Box as="a" href="#" aria-label="GitHub" rounded="full" w={8} h={8} bg="gray.800" display="flex" alignItems="center" justifyContent="center">
                <Icon boxSize={4} viewBox="0 0 24 24">
                  <path fill="currentColor" d="M12,2A10,10 0 0,0 2,12C2,16.42 4.87,20.17 8.84,21.5C9.34,21.58 9.5,21.27 9.5,21C9.5,20.77 9.5,20.14 9.5,19.31C6.73,19.91 6.14,17.97 6.14,17.97C5.68,16.81 5.03,16.5 5.03,16.5C4.12,15.88 5.1,15.9 5.1,15.9C6.1,15.97 6.63,16.93 6.63,16.93C7.5,18.45 8.97,18 9.54,17.76C9.63,17.11 9.89,16.67 10.17,16.42C7.95,16.17 5.62,15.31 5.62,11.5C5.62,10.39 6,9.5 6.65,8.79C6.55,8.54 6.2,7.5 6.75,6.15C6.75,6.15 7.59,5.88 9.5,7.17C10.29,6.95 11.15,6.84 12,6.84C12.85,6.84 13.71,6.95 14.5,7.17C16.41,5.88 17.25,6.15 17.25,6.15C17.8,7.5 17.45,8.54 17.35,8.79C18,9.5 18.38,10.39 18.38,11.5C18.38,15.32 16.04,16.16 13.81,16.41C14.17,16.72 14.5,17.33 14.5,18.26C14.5,19.6 14.5,20.68 14.5,21C14.5,21.27 14.66,21.59 15.17,21.5C19.14,20.16 22,16.42 22,12A10,10 0 0,0 12,2Z" />
                </Icon>
              </Box>
              <Box as="a" href="#" aria-label="Telegram" rounded="full" w={8} h={8} bg="gray.800" display="flex" alignItems="center" justifyContent="center">
                <Icon boxSize={4} viewBox="0 0 24 24">
                  <path fill="currentColor" d="M9.78,18.65L10.06,14.42L17.74,7.5C18.08,7.19 17.67,7.04 17.22,7.31L7.74,13.3L3.64,12C2.76,11.75 2.75,11.14 3.84,10.7L19.81,4.54C20.54,4.21 21.24,4.72 20.96,5.84L18.24,18.65C18.05,19.56 17.5,19.78 16.74,19.36L12.6,16.3L10.61,18.23C10.38,18.46 10.19,18.65 9.78,18.65Z" />
                </Icon>
              </Box>
            </HStack>
          </VStack>
          
          <VStack align="flex-start" spacing={3}>
            <Text fontWeight="bold" fontSize="lg">Quick Links</Text>
            <Link color="gray.400" _hover={{ color: "purple.300" }}>Home</Link>
            <Link color="gray.400" _hover={{ color: "purple.300" }}>About Us</Link>
            <Link color="gray.400" _hover={{ color: "purple.300" }}>Pricing</Link>
          </VStack>
          
          <VStack align="flex-start" spacing={3}>
            <Text fontWeight="bold" fontSize="lg">Support</Text>
            <Link color="gray.400" _hover={{ color: "purple.300" }}>Help Center</Link>
            <Link color="gray.400" _hover={{ color: "purple.300" }}>FAQ</Link>
            <Link color="gray.400" _hover={{ color: "purple.300" }}>Contact Us</Link>
            <Link color="gray.400" _hover={{ color: "purple.300" }}>Terms of Service</Link>
            <Link color="gray.400" _hover={{ color: "purple.300" }}>Privacy Policy</Link>
          </VStack>
          
          <VStack align="flex-start" spacing={4}>
            <Text fontWeight="bold" fontSize="lg">Newsletter</Text>
            <Text color="gray.400">Subscribe to receive updates about new features and releases.</Text>
            <Flex width="100%">
              <Input 
                placeholder="Your email" 
                required
                bg="gray.800" 
                border="1px solid"
                borderColor="gray.700"
                _focus={{ borderColor: "purple.500" }}
                borderRadius="md"
                borderRightRadius="none"
              />
              <Button 
                onClick={handleSubmit}
                colorScheme="purple"
                borderLeftRadius="none"
                px={4}
              >
                <EmailIcon />
              </Button>
            </Flex>
          </VStack>
        </SimpleGrid>
        
        <Divider borderColor="gray.800" my={6} />
        
        <Flex 
          direction={{ base: "column", md: "row" }} 
          justify="space-between" 
          align={{ base: "center", md: "center" }}
          color="gray.500"
          fontSize="sm"
        >
          <Text>© 2025 CryptoLab. All rights reserved.</Text>
          <HStack spacing={4} mt={{ base: 4, md: 0 }}>
            <Link _hover={{ color: "purple.300" }}>Privacy</Link>
            <Link _hover={{ color: "purple.300" }}>Terms</Link>
            <Link _hover={{ color: "purple.300" }}>Cookies</Link>
          </HStack>
        </Flex>
      </Container>
    </Box>
  );
};

export default Footer;